...
void main(int argc, char *argv[]) {
    ...
    if (argc == 2) {
        ...
        printf("Use: timeprofiles [thread_count] [dt]\n");
        printf(
            "Will use default time profile and use thread_count=%d, with dt=%lf for %lu steps and %d steps per table "
            "entry\n",
    ...
    for (istep = 1; istep < integration_steps; istep++) {
        ...
        if ((istep % steps_per_idx) == 0) {
            ...
            //        (istep<integration_steps), tsize, idx, istep, time, faccel(time), VelProfile[idx],
            //        PosProfile[idx]);
        }
    }
    ...
    clock_gettime(CLOCK_MONOTONIC, &start);
    VelStep = 0.0;
    VelProfile[0] = VelStep;
    PosStep = 0.0;
    PosProfile[0] = PosStep;
    time = 0.0;
...
double faccel(double time) {
    ...
    //
    // If time in table is always 1 second apart, then we can simplify since (timeidx_next - timeidx) = 1.0 by
    // definition here
    ...
    return (
        ...
        //
        // I.e. accel[time] = accel[timeidx] + ( (accel[timeidx_next] - accel[timeidx]) / ((double)(timeidx_next -
        // timeidx)) ) * delta_t
        //
