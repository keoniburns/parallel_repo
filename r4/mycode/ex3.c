...
//     // This should be the summation of DefaultProfile, which should match the spreadsheet for a train profile for
//     dt=1.0 MPI_Reduce(&local_sum, &g_sum, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD); if (my_rank == 0)
//     printf("\nRank 0 g_sum = %lf\n", g_sum);

//     MPI_Barrier(MPI_COMM_WORLD);

//     // DISTRIBUTE RESULTS: Now to correct and overwrite all default_sum, update all ranks with full table by
//     sending
...
//             // Adjust so initial condition is ending conditon of prior sum - SUPER IMPORTANT to adjust initial
//             condition
//             // offset
...
//     // This should be the summation of the sums, which should match the spreadsheet for a train profile for
//     dt=1.0 MPI_Reduce(&local_sum, &g_sum, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD); if (my_rank == 0)
//     printf("\nRank 0 g_sum = %lf\n", g_sum);

//     // DISTRIBUTE: Finally correct and overwrite all default_sum_of_sums, update all ranks with full table by
//     sending
//     // portion of table from each rank > 0 to rank=0, to fill in missing default data
//     if (my_rank != 0) {
...
//     } else {
//         for (int q = 1; q < comm_sz; q++) {
//             MPI_Recv(&default_sum_of_sums[q * subrange], subrange, MPI_DOUBLE, q, 0, MPI_COMM_WORLD,
//             MPI_STATUS_IGNORE);
